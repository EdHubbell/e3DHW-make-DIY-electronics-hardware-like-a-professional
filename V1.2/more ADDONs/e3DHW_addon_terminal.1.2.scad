...
/**
 @file e3DHW_addon_terminal.1.2.scad
...
@par MAMMUT series
  ...
  Few design paramaters are required: N (number of terminals) and the cable section. The correct core mammut is selected on MAMMUT array and the connector is designed.
\li <b>serie cubeMammut-HT(\c add_cubeMammut(type = HT)) </b> \n
...
\li <b>serie cubeMammut-FT(\c add_cubeMammut(type = FT)) </b> \n
\image html cubemammutFT.jpg
 Like cubeMammut-HT, now the connector is full protected. 
\li <b>serie simpleMammut-HT (<tt>add_simpleMammut(type = HT)</tt>) </b> \n
\image html simpleHT.jpg
This serie (Half Type) is for box connections: the front, with a place for Dymo label, is out of the box and half mammut is naked inside the box.
\li <b>serie simpleMammut-FT (<tt>add_simpleMammut(type = FT)</tt>) </b> \n
\image html simpleFT.jpg
This serie (Full Type) is for boards: the front is for external wires, and the connector is protected. 
\li <b>serie simpleMammut-DT (<tt>add_simpleMammut(type = DT)</tt>) </b> \n
...
@note  The mammut data are in file e3DHW_mammut_data.1.2.scad. Keep it updated with correct measurements of mammuts you use.
...
@par dependences
  This library requires:
      ...
      \li \c e3DHW_mammut_data.1.2.scad
  ...
  To use this library you must add the following lines to your code:
      ...
      \li  <tt> include <e3DHW_addon_terminal.1.2.scad> </tt>
...
*/
//! @privatesection
include <e3DHW_mammut_data.1.2.scad>

//! @publicsection
// constants used for type - don't change it
HT =  0;   ///< half size type: to put 1/2 naked mammut inside a box.
...
/**
  @fn add_cubeMammut(n=undef, mmq=1, len=0, type = FT, step= undef, space = undef, exBar = 2, exScr=0, x= 0, y=0, rot =norot )
   Thinny mammut strip for internal connections.
   Connector with 2 flawors: 
      \li HT - half type serie
      \li FT - full type serie

  This connector is simple and very adaptable. That make it a good start point for more complex connectors.
  @param  n: number of connectors required on strip.
  @param  mmq: cable section in mmq.
  @param len: total height (on z axis, the connector is printed in vertical)/n
   If len is less than the minimum, len is adjusted to minimum (without warning). If larger, connector is centered.
...
/**
@fn get_cubeMammutSize( mmq=1, type = FT, exBar = 2)
...
/** 
   @fn carve_cubeMammut(n=undef, mmq=1, len=0, type = FT, step= undef, space = undef, exBar = 2, exScr=0, x= 0, y=0, rot =norot )
...
//! @publicsection
/**
@fn add1_terminalM3Block(n= undef, len= 0, x=0, y=0, rot = norot)
...
/**
@fn carve2_terminalM3Block(n= undef, len= 0, x=0, y=0, rot = norot)
...
/**
@fn do_nakedModel(idx, exBar=0,exScr=0, half=false)
