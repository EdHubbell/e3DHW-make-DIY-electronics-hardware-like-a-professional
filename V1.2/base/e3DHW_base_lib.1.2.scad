...
For a general overview of \b e3DHW \b project see <a target="_blank" href='https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/e3DHW_HOWTO_en.pdf'> https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/e3DHW_HOWTO_en.pdf</a>.
  (originale in italiano: <a target="_blank" href=https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/e3DHW_HOWTO_it.pdf> https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/e3DHW_HOWTO_it.pdf</a>). \n

The OpenScad implementation uses a growing set of full parametric libraries, all named \c e3DHW_xxxxx.scad.  \n
The \ref e3DHW_base_lib.1.2.scad "base library" generates a hollow or solid <b>main board</b> of any shape, to be used as structural support for electronics modules and devices.\n
Also, \ref e3DHW_DIN_boxes_lib.1.2.scad "DIN boxes library" contains <b>box</b> modules, to be used alone or inside a DIN rail enclosure: on boxes, ADDONs can be placed on any plane: top, bottom, lateral.
...
<i>All OpenScad libraries are designed to be very easy to use in your projects:</i> 
   ...
   \li the use of Doxigen gives us the complete <b>e3DHWref.1.2.chm</b><a href="https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/OpenScad%20libraries/e3DHWref.1.2.chm">https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional/blob/master/OpenScad%20libraries/e3DHWref.1.2.chm</a>, a help indispensable for the e3DHW users. Download and read it with a help viewer.
...
@see The \b e3DHW \b reference file for all libraries is  \c e3DHWref.1.2.chm. About ADDON standardization see @htmlonly <a href='e3_d_h_w__addon__base_81_82_8scad.html#addonuse'>here</a> @endhtmlonly.


All e3DHW designs are experimental and may change. The last version can be found here: <a href="https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional>https://github.com/msillano/e3DHW-make-DIY-electronics-hardware-like-a-professional</a> 

@subsection stlintro STL check and repair
 The e3DHW libraries are manifold safe. But that doesn't mind you will get every time a perfect STL file. \n
 ...
 To check a STL file I fallow the workflow:
  <OL> <li>Check the STL using \b KISSlicer: so you can see where the problems are (if present, color-coded).
      ...
      <UL> <li>  Try to change a little the design in \b OpenScad (changing parameters ? fill ? small rotation ?), then create a new STL
           <li>  or try to fix it using \b netfabb (<i>steps: apply automatic repair, remove old part, Export part as STL</i>)
...
I would like to apologize for my bad English, it is not my native language. 
*/

/**
 @file e3DHW_base_lib.1.2.scad
...
@par useful functions
  In this library are also some general-purpose functions, public for developer convenience.
...
@par dependences
 ...
 @version 0.1.2    29/07/2019 Bugs correction. 
                   Added: \c do_nuthole(), get_arrayOK
...
module polyBase(vertex, holes = undef, fill= DEFAULTFILL, thickness =BOARDTHICKNESS, x=0, y=0, rot = norot, roundc = _slabcorner){
   assert(get_arrayOK(vertex, 2, 3), "test on array failed");  // mandatory
...
/**   
   ...
   Easy module, without arrays. See picture for parameter definitions.
...
/**
  ...
  General-purpose module to do poly borders and lateral face for a board.
  Generates a lateral border from the vertex array (rounds corners).
  @param  vertex an array of points: [[p1.x, p1.y],[p2.x,p2.y],...], same as polyBase().
  @param  height border height (vertical, default BOARDTHICKNESS)
  @param  width border thickness (default _slabborder)
...
/**
  ...
  Generate simple holes, any diameter, or special holes for 3M bolts and self-tapping screws.\n
  ...
  @param  d if positive, it is the hole diameter
  if 0 or negative it is a coded special hole:
...
// ------------------- general-purpose array management
  /**
  @fn get_arrayOK(v, s =1, n=1)
  Test function for mandatory 'arrays of arrays' parameters.
  General-purpose function to verify arrays of points 2D [x,y] or arrays of holes [x,y,d].
  @param v  an array
  @param d  elements size required (points = 2, holes = 3)
  @param n  minimum number of elements in an array (polygons: at least 3 points).
  @returns true if the array is as required, else message and abort.
  */
  function get_arrayOK(v, s =1, n=1) = ( 
   ...
   union(){
       ...
       if(!is_undef(holes))
          for (n =[0:1:len(holes)-1]) if(!is_undef(holes[n].z))translate([holes[n].x,holes[n].y,0])
             cylinder(r=_slabHoleClearance, h=s);
       } // union ends
   if(!is_undef(holes))
