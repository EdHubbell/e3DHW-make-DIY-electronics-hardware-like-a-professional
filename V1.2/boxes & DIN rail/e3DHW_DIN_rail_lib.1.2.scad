...
*/
/**
  @file e3DHW_DIN_rail_lib.1.2.scad
   ...
   \li <i> Screw</i> (SC): Strong fix, it is better to use bolts with hex socket, because in small spaces it is easier to use an Allen key than a screwdriver. A 3x9 washer distributes the strain.
...
<hr>
   In total <b>7 parametric DIN clips</b> are defined in this library. They can meet different needs:\n
     <i>thee are standalone things</i> ready to use, fixing a loan like PCBs with screws. The size \c d is the distance of the holes. \n
   @image html dinPCB.jpg
     <i>four are building blocks</i> with a flat top, to be used in larger projects. For that clips  \c d is the total clip size.\n
...
@note  The Screw clips are sturdier than Spring clips. But the Screw clips are less easy to fix when a cable duct is 5 cm from the rail. For that sometimes the rails are populated off the cabinet, or you can not tighten the screw after clicking. A good use of screw clips is as fixing terminals.

@htmlonly <a name='clipuse' ></a> @endhtmlonly
@par library use
      All clips are totally compatibles, so in your project you can change the clip without to change the code.
   ...
   \li This code centers all clips so you can try at runtime any clip:
 @code
 ...
 @endcode
   ...
   \li The structure of this library allows clips interchangeability and the addition of news clips to the collection.
 @htmlonly <a name='clipusenote' ></a> @endhtmlonly
@note Of course, the 7 clips are different, about clip force and minimal dimensions: every application will have the best clip. Becuse the clips are fully compatible, you can try more clips then chose without changing the design ( see examples). \n For small separators it is best \c NSS (\c h min = 1), for strong terminal it is required a screw clip (\c h min = 9), while for DIN boxes it is better a very flat clip like \c MSX (\c h min 26). \n
   ...
   In this library, the function get_Clip() implements this strategy: having as default a couple like ["SPF","MSX"] the function returns MSX if h >= 26, otherwise returns SPF. So the code for boxes will works from small boxes to bigger ones, giving whenever the best solution.
...
@par useful functions
  In this library are also some public utilities: getters for all clips properties, like get_clipCount(), get_position(), is_in() etc.;.helper functions for DIN boxes: get_H(), get_Clip().


@par dependences
  This library requires:
      \li \c MCAD/polyholes.scad
      \li \c e3DHW_hardware_data.1.2.scad> </tt>
...
// code |           human| min h| min d| rail2top|   xm|    xq| extra_d
  ["SCB",    "BasicScrew",     9,    28,     13.5,    0,  10.0,    6.0],
...
// generic access functions, 'key0' is used as index.
// lookup function (associative array): given the key0 (like "SBC") returns the value in col 
function _valueGetter(key0, col, table = _DINCLIPS) =(table[get_position(key0,table)][col]);
...
@fn do_DINClip(clip, h, d, x=0, y=0, rot = [0,0,0])
 ...
 @param d for "SCB","SPC","SCC" it is the PCB mount holes distance, else is total size. See get_ClipLength().
*/
...
utility: gets the parameted \c d to be used in get_Clip() to have a \c width top.
...
/**
 ...
 The smallest screw DIN clip, only 36 mm top, only 11,5 mm rail to top. You can use it with a PCB, or other flat things, e.g. a box, tied with self-tapping screws./n
   Used as symmetrical and interchangeable clip, min width is 46 mm.
...
//public getter functions for _DINCLIPS: Clips data table.
function get_clipCount() = ( len(_DINCLIPS)); ///< For clips: returns the number of clips available

function is_in(key0,table = _DINCLIPS) = (is_num(search(key0,table)[0])); ///< utility: test the presence of \c key0 element on col0: returns \c true/false

function get_Key0(i, table = _DINCLIPS) = table[i][0];    ///< returns the key0(i)

function get_position(key0, table = _DINCLIPS) = (assert(is_in([key0],table), "key code not valid") search([key0],table)[0]); ///< given the key0 returns a row number (0..len()-1) 

function get_name(clip) =     (_valueGetter(clip, 1));  ///< returns the name of the DIN clip

function get_min_h(clip) =    (_valueGetter(clip, 2));  ///< returns the min. height

function get_min_d(clip) =    (_valueGetter(clip, 3));  ///< returns the min. width

function get_rail2top(clip) = (_valueGetter(clip, 4));  ///< returns distance rail/top, y axis

function get_xm(clip) =       (_valueGetter(clip, 5));  ///< returns m in equation xc = m*d + q

function get_xq(clip) =       (_valueGetter(clip, 6));  ///< returns q in equation xc = m*d + q, used by get_ClipTrnsX().

function get_exd(clip) =      (_valueGetter(clip, 7));  ///< returns extra value (when d is screws distance), used by get_ClipWidth().
