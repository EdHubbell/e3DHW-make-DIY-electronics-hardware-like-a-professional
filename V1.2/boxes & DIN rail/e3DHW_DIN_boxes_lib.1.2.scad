...
*/
/**
@file e3DHW_DIN_boxes_lib.1.2.scad
Contains parametric general-purpose boxes and separators and boxes for DIN rail. 
@par General-purpose boxes
   In this library are 2 modules building boxes: a cuboid box rectangleBox() and a universal do_standalone_polyBox(). This one has the base polygonal, defined by an array of points.\n
   Both boxes have a lid fixable in different ways. The cover and the bottom can become lateral faces turning the box, so the panel can be oblique.\n
   The cover and/or the bottom can be perforated.\n
  These boxes are designed to be used as a container for electronic devices when a custom printed box is preferable to a commercial container.
   The bottom, the lid and also a lateral face can be used as a panel. Using ADDONs the DIY maker can get custom panels with holes for LDs and switches, rectangular openings for displays, etc... with hi precision without a mechanical workshop.

@par DIN separators
   Din separators have many functions:
         \li be a lid for open box and devices placed on DIN rail: same shape than box.
         \li logical/functional connector modules separator, sometimes with labels: a little bigger than the near modules
   ...
   So separators can be very thin or very thick. All separators with thickness over 3 mm are implemented as a box with a lid.\n
   In the library is a generic polyDINSeparator() and a specialized boxDINSeparator() that meets standard DIN boxes.

@par DIN boxes
  Some PCB or device can be used as they are after tied to a DIN clip. But if they have a user interface the better solution is to use a DIN box with TOP. The TOP panel is accessible in any DIN standard container or cabinet.\n
...
@par Polygonal DIN shapes: rules
    For great freedom, some DIN modules require an array of points for shape definition. 
   \image html dinarray.png
   The 2D shape MUST be placed as in the picture:
   \li  all \c y measurements reference to the top of DIN rail.\n
   \li  the clip top left point is on y-axis, position [0, get_rail2top(clip)].
   \li  the bottom line of shape MUST be on x-axis: blue line ([0,0], [0,D])(D= width + get_exd())
...
The basic shape, rectangle, is defined by the \c width: <i>small, medium or large</i>: \ref DINWIDTHS 85 mm, \ref DINWIDTHM 90 mm, \ref DINWIDTHL 100 mm.\n
...
The length is in mm for separators, and in <i>half-modules</i> for boxes (1 module = 18 mm) see get_H().
...
@par Lid styles
  The lid can match the box in different styles, defined by a set of constants:\n
...
4 - \ref LSIN    the lid is inside the box sides.\n
\image html toplid.jpg
The Lid stile LSOUT is used when the lid is the panel, so it covers the box lateral walls (e.g. standalone boxes).\n 
   The stile LSIN is preferable when the panel is the bottom or a lateral side, car the box now hides the lid (e.g. a rotated standalone box, DIN boxes).\n
...
@par useful functions
  There are also many public utilities in this library . All are useful to design custom boxes or DIN separators.\n Many functions concern the management of arrays: get_squareArray2(), get_squareArray3(), get_DINvertex(), get_cut2_shape(). \n
  Some functions are about tower definitions: get_towerRadius(), get_towerDistance(), get_DINvertex2tower(), get_tower2holes(). \n
  At last do_verifyArrays() is a fast test of shapes and towers.
...
@par dependences
    This library requires:
            ...
            \li \c e3DHW_DIN_rail_lib.1.2.scad
    ...
    To use this library you must add the following line to your code:
            ...
            \li  <tt> include <e3DHW_DIN_rail_lib.1.2.scad></tt>
            \li  <tt> include <e3DHW_DIN_boxes_lib.1.2.scad></tt>\n
...
OPTION_BOXPRINT = true; ///< true: all pieces at z=0, otherwise the lid is in place.
/**
A decreasing set of box thicknesses (printer and filament dependent).
1 - \ref BOARDTHICKNESS (mm 2-3) for big main boards and lids with many ADDONs (in  e3DHW_base_lib.1.2.scad )\n
2 - \ref TOPTHICKNESS (mm 1.8-2.5) for panels: the bottom of DIN boxes, the top if used as a panel.\n
...
LSNONE =100;  ///< @htmlonly <a href='#lidstyle'>lidStyle</a> @endhtmlonly. constant: no lid, no towers, only box
LSTOW  =101;  ///< @htmlonly <a href='#lidstyle'>lidStyle</a> @endhtmlonly. constant: no lid, only box + towers
LSOUT = 102;  ///< @htmlonly <a href='#lidstyle'>lidStyle</a> @endhtmlonly. constant: the lid is on top.
LSIN  = 103;  ///< @htmlonly <a href='#lidstyle'>lidStyle</a> @endhtmlonly. constant: the lid goes inside the walls of the box.
...
//! @publicsection 
 // -------------------------------  box standalone
  ...
  With many parameters, this is a generic module used by all other specialized modules.\n
  The shape is defined from a \c vertex array and an height. Independent thicknesses can be defined for cover, sides, bottom. A \c towerAHoles array defines the towers used to fix the lid with screws, \c lidAHoles array defines the lid holes (for towers, and maybe more). The final look is set by the \c lidStyle, top and bottom filling factors and the radius of the round corners.
@image html custombox.png
  ...
  @param round_box radius round box corners. Default _slabcorner (in  e3DHW_base_lib.1.2.scad ); max 3;
  */
  module do_standalone_polyBox(vertex, height = 20, lidAHoles = undef, lidStyle = LSOUT, lidThick = BOARDTHICKNESS, lidFill = 100, bottomThick=TOPTHICKNESS, bottomFill = 100, sideThick = NOTOPTHICKNESS, towerAHoles = undef, towerRadius=3, x=0, y=0, rot = norot, round_box=_slabcorner){
   ...
   if(! is_undef(lidAHoles))  // optional
      assert(get_arrayOK(lidAHoles, 3, 1), "test on array failed");
   assert ((lidStyle >= LSNONE) && (lidStyle <= LSIN),"lidStyle value not allowed");
   if(! is_undef(towerAHoles))  // optional
       assert(get_arrayOK(towerAHoles, 3, 1), "test on array failed");
...
/**
  ...
  companion lid module for do_standalone_polyBox(). Use it with the same parameters.
  */
...
/**
  ...
  Standalone box, an enclosure for DIY electronic projects.
  This is a general-purpose rectangular box, in two pieces: a base box and a lid board.
  As default, the cover is tied to the base using 4 self-tapping screws, placed at the 4 corners. \n
  The front panel can be the cover or the base: ADDONs can be added on the floor of the box and/or on the lid.  Lateral sides too can have ADDONS: see e3DHW_addon_box.1.2.scad.\n
  @image html rectbox.png
  If required the lid and/or the bottom can be perforated ( \c lidFill, \c bottomFill) for ventilation.
  @param length  the total box length, mandatory. Min variable (with defaults: 15).
  ...
  @param lidHole,towerHole @parblock the diameter/code used for the holes on the lid and the tower top. (see do_nuthole()).\n
...
/**
  @fn polyDINSeparator(vertex = undef, length = 2, width=xauto, uclip = xauto, lidThick = SEPARATORTHICKNESS, boxThick = SEPARATORTHICKNESS, fill = DEFAULTFILL, x=0, y=0, rot = norot)
   ...
   This kind of separator is used logical and functional separator between DIN modules (e.g. terminals) or as strong terminal (with a screw). For that the length is in mm. \n
  ...
  @param length the separator length [mm], accepts any value. 
  ...
  @param uclip accepts different options: @parblock
       \li:\c xauto: this module chooses between "NSS" or "SPF", based on length (default).
  ...
  Any clip can be used, so take care to different clips size limits.
	   @endparblock
  @param lidThick is the lid and box bottom thickness. Default \ref SEPARATORTHICKNESS
  @param boxThick  is the box side thickness. Default \ref SEPARATORTHICKNESS.
...
/**
  @fn boxDINSeparator(length, top = DINTOP, width= DINWIDTHM, leftless=0, rigthless=0, uclip = xauto, lidThick = SEPARATORTHICKNESS, boxThick = SEPARATORTHICKNESS, fill = DEFAULTFILL, x=0, y=0, rot = norot)
   This kind of separator is used as a lid for open DIN boxes, and as filler to meet container dimensions. For that the length is in mm. 
...
@fn  simpleDINBox(size_hm, top = DINTOP, width= DINWIDTHM, leftless=0, rigthless=0, uclip = xauto, lidHole = -7, lidThick = TOPTHICKNESS, lidFill = 100, towerHole = -58, bottomThick = TOPTHICKNESS, bottomFill = 100, boxThick = NOTOPTHICKNESS, lidStyle = LSIN, x=0, y=0, rot = norot)
  ...
  The box can contain one or more horizontal or vertical PCBs (addon PCB rails in e3DHW_addon_boxes.scad) and any ADDON on the lid or TOP.
  ...
  Accepts only symmetrical DIN clips.
  @param lidHole,towerHole @parblock the diameter/code used for the holes on the lid and on the tower top. (see do_nuthole()).\n
       The default is for self-tapping screws (\c lidHole: -7 = \c conic \c head and \c towerHole: -58 = \c pilot \c hole).\n
 	    <tt>lidHole == 0</tt> minds no holes on the lid.\n
       <tt>towerHole == 0</tt> minds no towers: you must use custom ways to tie the cover.</i>
       @endparblock
...
@fn bigDINBox(size_hm, base_width=50, top = DINPOT, uclip = xauto, boardThick = BOARDTHICKNESS, boardFill = DEFAULTFILL,  topFill = 100,  topThick = TOPTHICKNESS, boxThick = NOTOPTHICKNESS)
   This big board consists of four pieces assembled by screws.
 ...
 \image html bigDINbox.png
   ...
   @param uclip here you can use standalone strong clips: "SCB" the smallest, "SPC" with spring or "SCC".
   @param boardThick is the main board thickness. Default \ref BOARDTHICKNESS
   ...
   @param topFill is the fill factor for the TOP. Default 100, because it is the panel. If the box is without TOP. can be perforated for aeration.
*/
...
utility: makes an array2 (points) for rectangular shapes.
*/
...
/**
#@fn get_squareArray3 (b, h, offx=0, offy=0, d=0)
utility: makes an array3 (holes, towers) for rectangular shapes.
*/
function get_squareArray3 (b, h, offx=0, offy=0, d=0)=(
...
function get_towerDistance(towerHole, lidStyle = LSIN)=
  ...
  utility: development test for polygon array and towers positions. Use with array definitions only.
  */
module do_verifyArrays(vertex, towers = undef){
   assert(get_arrayOK(vertex, 2, 3), "test on array failed");
   if(!is_undef(towers))
         assert(get_arrayOK(towers, 3, 3), "test on array failed");
   difference(){
      union(){
