...
*/
/**
 @file e3DHW_DIN_modules_A.1.2.scad
 Collection of ready to print and to use DIN modules.
 Here you can found:
  \li dinBasicTerminalBlock() serie. It uses simple double mammuts, size variabile, wire variable, as defined in e3DHW_addon_terminal.1.2.scad. \n
  \image html dinterminalbasic.jpg
   The family is completed by end plates, dinBasicEndPlate() and strong end brackets with a screw, dinBasicEndBrackets().
...
@note: Terminal are designed to require as parameter only the cable section. The correct mammut core is chose from e3DHW_mammut_data.1.2.scad.
...
@par dependences
  This library requires:
      ...
      \li \c e3DHW_DIN_boxes_lib.1.2.scad
      \li \c e3DHW_addon_terminal.1.2.scad
  ...
  To use this collection you must add the following lines to your code:
      ...
      \li <tt> include <e3DHW_DIN_modules_A.1.2.scad> </tt>\n
...
// return parametric vertex  array: uses some getter protected functions in e3DHW_addon_terminal.1.2.scad
...
//  uses some getter protected functions in e3DHW_addon_terminal.1.2.scad
...
Full parametric, auto-select the correct mammut as defined in e3DHW_mammut_data.1.2.scad.\n
...
Slim endplate for basic blocks.
...
It can be used also as a logical/functional separator car it accepts a Dymo 9 label.
...
Space and filament saving DIN terminal, structured as a Hanoi tower.
...
Full parametric, selects the correct mammut as defined in e3DHW_mammut_data.1.2.scad\n
...
@fn dinTowerEndPlate(n, mmq=1, s=2)
 Parametric endplate, lid for tower blocks.
...
Strong end bracket with a screw for tower blocks.
It can be used also as a logical/functional separator car it accepts a Dymo 9 label.
Required because tower blocks and end tower plates use weak NSS clips.
@param n number of floors, [1,2,3]
@param mmq wire section (mmq)
